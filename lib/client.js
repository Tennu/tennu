 ...
 */
 var Client = function (config, dependencies) {
    ...
    var messageHandler = new di.MessageHandler(client, client._logger, client._socket);
    ...
    var commandHandler = new di.CommandHandler(config, client._nickname, client._logger);
    ...
    var require = client._modules.require.bind(client._modules);
...
Client.prototype.nick = function () {
    if (arguments.length === 0) {
        var errormsg = "tennu.Client#nick() nullary method is removed. Use tennu.Client#nickname() instead."
        this.error(errormsg);
