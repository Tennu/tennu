...
NRC.prototype._onString = function (string, listener) {
    ...
    string.split(" ").forEach(function (event) {
        if (event[0] === '!') {
            ...
            events.EventEmitter.prototype.on.call(that, event, listener);
        }
    });
};
...
NRC.prototype._onceString = function (string, listener) {
    ...
    string.split(" ").forEach(function (event) {
        if (event[0] === '!') {
            that._commander.on(string, listener);
        } else {
            events.EventEmitter.prototype.once.call(that, event, listener);
        }
    });
};
...
NRC.prototype.require = function (module) {
  ...
  this._loadedModuleNames.append(module.name);

  // HANDLERS
  for (var handler in module.handlers) {
    var events = handler.split(" ");
    var listener = module.handlers[handle];
  ...
  // EXPORTS
  this.modules[module.name] = module.exports || {};
  this.modules[module.name].name = module.name;
...
NRC.prototype.isModule = function (name) {
  return this._loadedModules.indexOf(name) !== -1;
};
