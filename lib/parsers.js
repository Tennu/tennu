 ...
 * and "nick@host"].
 */
parsers.prefix = function (prefix) {
  var exclamation = indexOf(prefix, "!");
  var at = indexOf(prefix, "@");
  var dot = indexOf(prefix, ".");

  if (!exclamation && at && !dot) {
    return "server";
...
/**
 * Takes an ircmsgEvent of a privmsg and sets it up with useful shortcuts
 * and more importantly, changes it to a command-type event should it be one.
 * @public
 * @param {ircmsgEvent} eventEvent with a name of 'PRIVMSG'
 * @param {string} nick Nick of bot.
 * @param {string} comchar Character to check at beginning of message to see
 * if a command or not. By default, "!".
 */
parsers.privmsg = function (event, nick, comchar) {

  // Every message and command gets these three.
  event.messenger = parsers.hostmask(event.prefix, "nick");
  event.channel = event.params[0];
  event.msg = event.params[1].trim().replace(/\u0003\d?\d?,?\d?\d?/g, "");
  event.query = (event.channel === nick);

  // Check to see if this is a command.
  if (event.channel === nick) {
    event.name = 'COMMAND';
  } else if (event.msg[0] === comchar[0]) {
    event.msg = event.msg.substr(1);
    event.name = 'COMMAND';
  } else if (event.msg.indexOf(nick) === 0) {
    event.msg = event.msg.substr(event.msg.indexOf(" ") + 1);
    event.name = 'COMMAND';
  }

  if (event.name === 'COMMAND') {
    // Store original parameters and name in "o"riginal slots.
    event.oparams = event.params;
    event.oname = event.name;
    event.params = event.msg.split(' ');
    event.name = "C-" + event.params.shift().toUpperCase();
  }
};
